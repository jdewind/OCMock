//---------------------------------------------------------------------------------------
//  $Id: OCMConstraintTests.m 57M 2010-11-30 05:43:35Z (local) $
...
- (void)testCapturesArguments {
  OCMCaptureConstraint *constraint = [[[OCMCaptureConstraint alloc] init] autorelease];
	
  STAssertTrue([constraint evaluate:@"the argument"], @"Should have accepted argument");
  STAssertEqualObjects(@"the argument", [constraint argument], @"Should have captured the argument");
}

- (void)testThrowsAnExceptionIfArgumentHasNotBeenCaptured {
  OCMCaptureConstraint *constraint = [[[OCMCaptureConstraint alloc] init] autorelease];
	STAssertThrows([constraint argument], @"Should have thrown an exception");  
}

#if NS_BLOCKS_AVAILABLE

- (void)testCopiesArgumentIfItIsABlock {
  OCMCaptureConstraint *constraint = [[[OCMCaptureConstraint alloc] init] autorelease];
  
  // To ensure we get a __NSStackBlock__
  int referenceVariable = 1;
  int (^aBlock)(void) = ^{
    return referenceVariable;
  };
  
	[constraint evaluate:aBlock];
  
  int (^capturedBlock)(void) = [constraint argument];
  
  STAssertTrue(capturedBlock != aBlock, @"Blocks should not be the same");
}
